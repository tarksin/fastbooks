...
from app.forms import LoginForm, AddExpenseForm, AddBankItemForm, AddExpcatForm, AddTodoForm, AddProjectForm, \
    GetSalesTaxForm, ReportForm, RegistrationForm
...
def index():
    links = [
        {'english': "seventhnormal.com", 'url': "http://seventhnormal.com"},
        {'english': "p680plus.com", 'url': "http://p680plus.com"}
    ...
    return render_template('index.html', headline='K-State Research Centers', centers=centers, links=links)
...
def get_sales_tax():
    ...
    if request.method == "POST":
        ...
        display_string1 = "The sales tax on ${} at {} in {}, {} is ${}.".format("{:.2f}".format(float(purchase_amount)),
                                                                                line1, city, state,
                                                                                "{:.2f}".format(tax2['totalTax']))
        ...
        return render_template('get_sales_tax.html', form=form, header_text="Get sales tax amount",
                               display_string1=display_string1, display_string2=display_string2)
    return render_template('get_sales_tax.html', form=form, header_text="Get sales tax amount")
...
def login():
    ...
    if form.validate_on_submit():
        # flash('R 369: {}'.format(form.username.data))
        # flash('R 375')
        yes_no = User.user_exists(form.username.data)
        # flash(yes_no)
        if yes_no:
          #  flash('R 379 valid username: {}'.format(form.username.data))
            pwh = User.get_pwhash(form.username.data)
          #  flash('R 383')
          #  flash(pwh)
          #  flash('R 385')
            if check_password_hash(pwh, form.password.data):
           #     flash('R 382 valid password: {}'.format(form.username.data))
                session['logged_in'] = form.username.data
                session['logged_id'] = User.get_id(form.username.data)
                return redirect(url_for('index'))
            else:
                flash('R387 Invalid password for {}'.format(form.username.data))
                return redirect(url_for('login'))
        else:
            flash('R390 Invalid username: {}'.format(form.username.data))
            return redirect(url_for('login'))

            # return redirect(url_for('login'))

        # if user is None:
        # if not data_u:  # 'username'] == form.username.data:
        #


        # else:
        #     flash('R 375 OK  Valid username and password(?): {}'.format(form.username.data))
        #     session['logged_in'] = form.username.data
        #     # login_user(user)
        #     return redirect(url_for('index'))
...
def register():
    ...
    if form.validate_on_submit():
        flash('424 form.username.data: {}'.format(form.username.data))
        ...
        if user_exists:  # is None or not user.check_password(form.password.data):
            flash('428 User account already exists')
            return redirect(url_for('register'))
        else:
            flash('431 new username is {}'.format(form.username.data))
            pw_hash = generate_password_hash(form.password.data)
            user = User(form.username.data, form.firstname.data, form.lastname.data,  form.email.data,
                        pw_hash)
            ...
            flash('444 Registered: {}'.format(form.username.data))
            return redirect(url_for('index'))
